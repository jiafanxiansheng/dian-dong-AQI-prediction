...
| 排名 | 模型 | R² | RMSE | 耗时 | 类型 |
|------|------|-----|------|------|------|
| 1 | **Prophet** | **0.914** | 4.15 | 1.4s | 时间序列分解（含 regressors） |
| 2 | CatBoost | 0.456 ±0.095 | 12.87 | 9.7s | 梯度提升·Boosting |
| 3 | RandomForest | 0.450 ±0.074 | 12.96 | 18.2s | 集成学习·Bagging |
| 4 | LSTM | 0.159 ±0.082 | 16.00 | 63.0s | 深度学习·循环网络 |

> **结论**：Prophet（含污染物 + 时间 regressors）R²=0.914 断崖式领先，RMSE 仅为树模型的 1/3。CatBoost 与 RandomForest 表现接近（R²≈0.45），说明梯度提升在纯表格特征上对 Bagging 无明显优势。LSTM R² 仅 0.16 且耗时最长（63s），在中等规模时序数据上不如专门的时序分解模型。最终选定 Prophet 作为主力预测引擎。
