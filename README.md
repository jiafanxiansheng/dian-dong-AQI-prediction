...
| 排名 | 模型 | R² 均值 | R² 标准差 | RMSE | 耗时 | 类型 |
|------|------|---------|-----------|------|------|------|
| 1 | **Prophet** | **0.907** | — | — | — | 时间序列分解 |
...
> **结论**：Prophet R²=0.907 断崖式领先，是第二名（CatBoost 0.456）的近两倍。两个树模型表现接近，说明梯度提升在纯表格特征上对 Bagging 无明显优势。LSTM R² 仅 0.16 且耗时最长（63s），在中等规模时序数据上不如专门的时序分解模型。最终选定 Prophet 作为主力预测引擎。
