...
在站点 **1916A**（曲靖环境监测站，93,410 条数据）上，基于统一 66 维特征集进行 5 折时序交叉验证，对比四类模型：

| 排名 | 模型 | R² 均值 | 类型 | 结论 |
|------|------|---------|------|------|
| 1 | **Prophet** | **0.907** | 时间序列分解 | ✅ 选为主力模型 |
| 2 | RandomForest | 0.450 ±0.074 | 集成学习·Bagging | 非线性能力弱于时序模型 |
| 3 | CatBoost | — | 梯度提升·Boosting | 调参成本高，收益有限 |
| 4 | LSTM | — | 深度学习·循环网络 | 训练成本高，小数据量易过拟合 |

> Prophet 最终胜出的关键原因：对缺失值鲁棒、自动捕捉季节性和节假日效应、训练高效，在逐时 AQI 时序预测场景下显著优于纯树模型和深度学习方案。CatBoost 和 LSTM 均因训练/调参成本与收益不成正比而未被采用。
