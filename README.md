...
```
历史数据 → 特征工程（66维：滞后 + 滚动统计 + 差分 + 周期编码）
          → 多模型对比评估（RandomForest / Prophet）
          → 5折时序交叉验证 → 选择最优模型（Prophet R²=0.907）
          → 6h / 12h 多步预测 → AQI 约束[0, 500]
          → JSON 返回
```
...
通过 `scripts/compare_models.py` 在站点上公平对比多个模型：

| 模型 | 类型 | R² | 说明 |
|------|------|-----|------|
| **Prophet** | 时间序列分解 | **0.907** | ✅ 选为主力模型 |
| RandomForest | 集成学习·Bagging | 对比中 | 非线性拟合能力强 |

> 对比实验基于统一特征集（66维时序特征），采用 5 折时序交叉验证，确保训练集/测试集时间顺序不泄露。
