...
│  │               predictor.py (预测引擎)              │   │
│  │  ┌─────────────────┐  ┌────────────────────────┐ │   │
│  │  │ ProphetModelWrap │  │  CatBoostModelWrapper  │ │   │
│  │  │ (当前主力模型)    │  │  (可选替代，框架已集成)  │ │   │
│  │  └─────────────────┘  └────────────────────────┘ │   │
│  │  特征工程 → 模型推理 → 多步预测(6h/12h) → 结果约束   │   │
│  └──────────────────────────────────────────────────┘   │
│                                                          │
│  ┌──────────────────────────────────────────────────┐   │
...
| 模型 | 类型 | 当前状态 | 适用场景 |
|------|------|----------|----------|
| **Prophet** | 时间序列分解 | ✅ 主力（7/7 站点） | 趋势 + 周期 + 节假日效应 |
| **CatBoost** | 梯度提升树 | 🔧 框架已集成，待启用 | 多特征非线性的复杂站点 |

系统设计了**可插拔模型架构**（`ProphetModelWrapper` / `CatBoostModelWrapper`），新增模型只需实现 `predict()` 接口并在 `SITE_MODEL_PREFERENCE` 中注册即可自动切换。

### 预测流水线

```
数据库读取 → 新鲜度检测(>2h触发爬虫) → 特征工程
                                              ├─ Prophet: 滞后特征 + 滚动统计(3窗口)
                                              └─ CatBoost: 滞后特征(1,2,3,6,12,23h)
                                                        + 滚动均值/标准差(3,6,12,24窗口)
                                                        + 差分特征(1,3,6,12h)
→ 模型推理 → AQI约束[0,500] → JSON返回
```
...
| `GET` | `/api/sites` | 所有站点信息 + 模型类型 |
| `GET` | `/api/aqi-levels` | AQI 等级定义（6 级） |
| `GET` | `/api/status` | 系统状态（模型摘要、上线时间、数据时效） |
...
│
├── services/                 # 核心服务（业务逻辑）
│   ├── predictor.py          # 预测引擎：双模型加载 · 特征工程 · 多步预测
...
│
├── scripts/                  # 离线工具
│   ├── train_prophet.py      # Prophet 训练流程
│   ├── train_catboost.py     # CatBoost 训练流程（含完整特征工程）
...
| 决策 | 选择 | 理由 |
|------|------|------|
| 预测算法 | Prophet | 对缺失值鲁棒，自动捕捉季节性，适合逐时 AQI 数据 |
| LLM 集成 | DeepSeek Chat | 中文能力强，API 兼容 OpenAI，成本低 |
| 数据爬取 | HTTP API (非 Selenium) | 0.1s vs 25s，免浏览器依赖，稳定性高 |
| 前端图表 | ECharts 5.5 | 开箱即用的时间序列交互图表 |
| 模型架构 | Wrapper 模式 | `predict()` 统一接口，添加新模型零侵入 |
