...
在站点 **1916A**（曲靖环境监测站，93,410 条数据）上，基于统一 66 维特征集进行 5 折时序交叉验证对比：

| 排名 | 模型 | R² 均值 | R² 标准差 | RMSE | 结论 |
|------|------|---------|-----------|------|------|
| 1 | **Prophet** | **0.907** | — | — | ✅ 选为主力模型 |
| 2 | RandomForest | 0.450 | ±0.074 | 12.96 | 非线性能力弱于时序模型 |

> Prophet 对缺失值鲁棒、自动捕捉季节性/节假日效应，在逐时 AQI 时序预测任务上显著优于纯树模型。对比基于 5 折时序交叉验证，确保训练集时间严格先于测试集，避免数据泄露。

<details>
<summary>其他站点 RandomForest 验证结果（点击展开）</summary>

| 站点 | R² 均值 | RMSE |
|------|---------|------|
| 1917A 烟厂办公区 | 0.461 | 13.95 |
| 2596A 监测站 | 0.430 | 14.41 |
| 2610A 州水务局 | 0.578 | 14.79 |
| 2611A 便民服务中心 | 0.606 | 14.37 |
| 3376A 南苑二区 | 0.449 | 12.39 |
| 3377A 曲靖师范学院 | 0.473 | 13.55 |
